#!/usr/bin/env bash
# init-project 리허설 (수동 실행 전용 — node --test 글롭에 포함되지 않는다).
# 실행: mise run init-rehearse   (또는 bash tests/init-project-flows.sh)
# 작업 디렉터리는 INIT_PROJECT_TEST_WORK_DIR 로 재지정할 수 있다.
# tests/ 에 있어 project-owned 로 분류되고 다운스트림에 배포되지 않는다
# (2026-07-31 감사 1-3: 이전 위치 .harness/scripts/setup 은 shared-manifest
# 에 실려 모든 다운스트림에 심링크로 배포됐다).
set -euo pipefail
...
case_empty_pull_then_init() {
  ...
  run_init_project "$repo" "codi-empty-smoke" "case1"
  ...
  assert_file "$repo/harness.lock"
  assert_contains "$repo/harness.lock" '"channel": "latest-minor"'
  ...
  assert_contains "$repo/.github/workflows/dependency-security.yml" "Restore successful full scan cache"
  ...
  if git -C "$repo" log --oneline -1 --pretty=%s | grep -q "^chore: 프로젝트 초기화 codi-empty-smoke$"; then
...
EOF
  ...
  run_init_project "$mono_repo" "codi-import-smoke" "case2"
...
case_wire_infisical_after_init() {
  ...
  run_init_project "$repo" "codi-wire-smoke" "case3-init"
  ...
  run_wire_infisical "$repo" "codi-wire-smoke" "case3-wire"

  assert_contains "$repo/apps/front/.infisical.json" "mock-project-id-codi-wire-smoke"
  assert_contains "$repo/apps/back/.infisical.json" "mock-project-id-codi-wire-smoke"
