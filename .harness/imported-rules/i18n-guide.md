...
2. **`codi-config.yaml`** — `language` field in `.agents/codi-config.yaml`
3. **Fallback** — English (en) if neither of the above is set

```yaml
# .agents/codi-config.yaml
language: ko  # ko, en, ja, zh, ...
```

## Translation Voice

When translating user-facing content, the `translation_voice` field in `.agents/codi-config.yaml` controls global rhythm and formality. It is applied on top of `codi-translator` content-type persona routing.
...
Workflows that translate user-facing content should respect this setting via the `codi-translator` skill rather than hardcoding a tone.
...
6. **Translation tasks** — for translating UI strings, docs, or marketing copy, use the `/codi-translator` skill
...
```
- **Response language follows `language` setting in `.agents/codi-config.yaml` if configured.**
```
