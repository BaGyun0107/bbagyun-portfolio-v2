...
Use the `codi-rule-authoring` skill for harness rule work so Codex/Claude
runtime mirrors and regression tests are considered before editing.
...
Layer selection criteria (compaction survival, persistence, trigger, scope,
override — Anthropic's official steering taxonomy) live in the
`codi-rule-authoring` skill: `references/steering-taxonomy.md`. Answer its
five questions before placing any new component.
...
tool input. Since 2026-07-06 that gap is partially closed: Codex official
lifecycle hooks exist, and `.codex/hooks.json` +
`.harness/hooks/codex-pretooluse.mjs` reuse `guardrails.mjs` for Bash blocks
on Codex (blocks only; non-blocking warnings remain Claude-only for now).
For checks not yet ported, put them in shared hooks and document the Codex
limitation in the common policy and Claude mirror.
