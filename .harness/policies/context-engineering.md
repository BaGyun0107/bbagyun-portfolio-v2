...
| Work state | `specs/<NNN-feature>/` (unchecked tasks.md + design artifacts) |
...
| Skill ownership rules | "Shared Skill Ownership" in `.harness/policies/guardrails.md` |
...
Planning Hub automation follows the same parity rule. Claude and Codex Stop
configuration call the shared `runPlanningSyncIfRelevant` adapter; runtime-specific
files only normalize payload and cwd. Stop is non-blocking and cannot replace
Planning Lock. CI runs the strict `planning:check` to cover direct edits and missing
local hooks. Generated hub output and automation logs are excluded from trigger
classification.
...
| WHY | background of architecture decisions, tradeoffs, exception rationale | `specs/<NNN-feature>/`, `ARCHITECTURE.md` |
...
- Do not restate formatting rules already enforced by ESLint, Prettier, or
  TypeScript.
- Do not state facts the agent can discover from the repository itself
  (file structure, obvious commands, framework defaults); spend entry-point
  lines on repo-specific gotchas and non-obvious constraints instead.
...
- A decision that only exists in chat is not source of truth; move it into
  a spec directory or a policy file.
...
- If an app needs an exception from shared policy, record the reason, owner,
  expiry condition, and safer compensating control in `docs/audits/` or an
...
2. Record which document is stale in `docs/audits/` or the review log.
