...
2. `permanent_deny`: operations an agent must never perform, such as
   `gh pr merge`
3. `approval_required`: dangerous operations (for example `rm -rf` or a
   production deploy); the canonical list lives in
...
broader one. Do not add deny rules that cover the harness package cache
(`~/.codi-harness/**`): lock-mode repos read rules/skills through symlinks
into that cache, and a deny there blocks tool-path reads through those
symlinks (specs/005-harness-packaging R4). Production work, secret access, data deletion, and Git history
...
The role is set via `CODI_AGENT_ROLE`. If it is not set, the `default` role
applies.
...
Playwright MCP is the shared browser-QA surface for both runtimes, registered
at the user level and pinned to `@playwright/mcp@0.0.79` (the pin's single
source of truth is `.harness/scripts/checks/mcp-registration-check.mjs`;
install/preflight/doctor all read it from there).

- Browser navigation, snapshots, screenshots, and form interaction against
  local/dev targets are normal-use (all roles).
- Driving production sites through the MCP browser follows the production
  rules above: read-only inspection is allowed, but any flow that submits
  data, sends messages, or completes payments on production requires user
  approval.
- Real-browser session cases (device auth, complex logins) use the
  claude-in-chrome path, not headless MCP.
