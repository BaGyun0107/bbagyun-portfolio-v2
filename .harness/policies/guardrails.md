...
This section is the source of truth for skill ownership.
`.claude/rules/skill-ownership.md` is the Claude-side operational mirror, and
`.harness/policies/update-policy.md` defers here — do not restate the rule at
length elsewhere.
...
files under that tree. `./harness update` overwrites
...
The hook also emits a non-blocking warning when a bulk in-place edit
(`sed -i`, `perl -i`, `awk -i inplace`, including `xargs`/`find -exec`
forms) targets `apps/**`. Field evidence (2026-08-13): batch script edits
passed lint five times while silently corrupting app code — multi-line
regex damage, missing paired inserts, function-boundary misses are
intent-level breakage lint cannot catch. Legitimate uses exist, so this
warns instead of blocking; prefer individual Edit-tool changes or verify
each touched file before committing. Like other non-blocking warnings,
this is Claude-only for now (the Codex adapter ports blocks only).
