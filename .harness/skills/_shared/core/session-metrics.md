...
- **Aggregation**: `codi stats` command summarizes trends
...
- `false_negative`, `severity_mismatch`: Recorded retroactively via `codi retro` or next session discovery
...
| Threshold | Action |
|-----------|--------|
| EA >= 30 | **TUNING SUGGESTED**: `codi retro` flags QA patterns for review |
...
- **Precise tokens**: Available only via `codi stats` post-hoc (parses CLI logs when supported)
