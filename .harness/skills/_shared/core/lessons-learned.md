...
Accumulated lessons are recorded in the project's committed lesson record —
the relevant `specs/<NNN-feature>/` directory for feature-scoped lessons, a
project-owned log under `docs/` for cross-feature lessons, or
`.harness/skills-local/` for skill-shaped lessons — NEVER in this
upstream-owned file — downstream writes to `.harness/skills/` are blocked and
overwritten by `./harness update`.
...
Once a trigger fires, add the lesson to the project's lesson record before
