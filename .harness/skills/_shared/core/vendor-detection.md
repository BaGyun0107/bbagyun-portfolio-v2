...
Then resolve the target vendor for each agent from `.agents/codi-config.yaml` (`agent_cli_mapping`, `default_cli`).
...
- If native invocation is not available for that agent, fall back to `codi agent:spawn`.
...
5. **CLI Fallback**: None of the above matched → use `codi agent:spawn`
...
| Codex CLI | Native custom agents in `.codex/agents/{name}.toml` via `codex exec "@agent ..."` when available, otherwise `codi agent:spawn` | JSON output |
| Gemini CLI | `.gemini/agents/{name}.md` native subagents via `gemini -p "@agent ..."` when available, otherwise `codi agent:spawn` | JSON output or MCP memory poll |
| Antigravity | Prefer `codi agent:spawn` unless a native role-subagent path is explicitly verified for the target vendor | MCP memory poll |
| CLI Fallback | `codi agent:spawn {agent} {prompt} {session} -w {workspace}` | Result file poll |
...
3. Otherwise, use `codi agent:spawn`
