...
work state — the feature's `specs/<NNN-feature>/` directory for Spec
Kit-managed work, otherwise the session's working notes — so a later attempt
does not repeat a discarded fix.
