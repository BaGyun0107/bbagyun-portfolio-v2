...
5. For UI work, check for the project design system before implementation:
   - token file: `apps/front/src/styles/tokens.css`
   - design principles: `docs/design-system.md`
   If both exist, read both on demand and treat them as the visual source of
   truth. If they are absent, use `.harness/imported-rules/design.md` as the
   shared default.
6. Determine whether work belongs in framework routes, components, wrappers, hooks, or styles.
...
- If backend contracts are missing, follow `.harness/policies/api-contract-first.md`:
  draft or request the contract first; never invent mock fields that have no
  basis in an agreed contract or the `apps/back` schema (stop and request instead).
...
- **Project design system**: `apps/front/src/styles/tokens.css` plus
  `docs/design-system.md` when present. Read both before UI work and use their
  semantic tokens and principles.
...
- **Design Tokens**: project-owned `apps/front/src/styles/tokens.css` (OKLCH,
  Tailwind v4 `@theme inline`) — never hardcode colors
