...
Project design-system priority:

1. If `apps/front/src/styles/tokens.css` exists, it is the token source of truth.
   Read it together with `docs/design-system.md` before styling UI.
2. If the project design system is absent, use the shared defaults in
   `.harness/imported-rules/design.md`.
3. Do not preload project token values into always-on context; load them
   on demand for UI work.

Map all colors to semantic tokens from the project design system. Never use raw
colors like `bg-white` or `text-gray-900`.
