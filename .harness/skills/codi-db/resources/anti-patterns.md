...
- **Raw SQL by habit**
  - Smell: raw SQL is used when the ORM or query builder can express the same query safely
  - Fix: prefer ORM/query builder; keep raw SQL only for justified cases and review it like code
- **Interpolated SQL**
  - Smell: user input or request data is concatenated into SQL strings
  - Fix: use bind parameters; for table, column, sort, and filter identifiers, use explicit allowlists
- **Unbounded write**
  - Smell: `UPDATE` or `DELETE` has no bounded `WHERE`
  - Fix: require scoped predicates, expected affected row count, transaction plan, and rollback path
- **Broad write predicate**
  - Smell: `WHERE 1=1`, weak status-only predicates, missing tenant/user scope, or cleanup writes without batching
  - Fix: add tenant/account/user scope, dry-run `SELECT`, limit/batch plan, and explicit approval for destructive bulk changes
- **NULL misuse**
...
- **Unsafe migration**
  - Smell: `DROP`, `TRUNCATE`, destructive `ALTER TABLE`, reset/drop commands, or generated migrations applied without review
  - Fix: require explicit approval, human-readable diff review, forward/backward or roll-forward strategy, lock/downtime assessment, and verification command
