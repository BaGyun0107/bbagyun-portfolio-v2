    ...
    steps:
      - uses: google-github-actions/release-please-action@v4
        with:
          ...
          release-type: node
          package-name: codi-harness
```

## Mise Tasks for Release Management

Use the target project's Node.js version here. Keep `node = "24"` only for
new Codi-scaffolded projects or projects that already target Node.js 24.
...
npm exec -- gh pr list --label "autorelease: pending" --state open
echo ""
echo "Latest releases:"
npm exec -- gh release list --limit 5
'''
...
npm exec -- gh pr list --label "autorelease: pending"
...
#!/usr/bin/env bash
echo "✅ Latest release:"
npm exec -- gh release view --json tagName,name,createdAt

echo ""
echo "📦 Assets:"
npm exec -- gh release view --json assets
'''
```
...
npm exec -- gh run list --workflow=release-please.yml --limit 5 2>/dev/null || echo "  No runs found"

echo ""
echo "4. Open release PRs:"
npm exec -- gh pr list --label "autorelease: pending" 2>/dev/null || echo "  No release PRs"
'''
```
