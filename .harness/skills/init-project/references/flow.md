...
Parse `origin` if it matches `github.com/<org>/<repo-name>.git`. If matched,
...
Question 5 — git history restart (only for a brand-new project starting from
a fresh harness clone):

- Confirm applying `--reset-git` (default: **yes**). A new project must not
  keep the harness repo's history/version branches. Skip only when the tree
  is the harness repo itself or an already-initialized project.

Record the answers in the active spec (`specs/<NNN-feature>/`) before proceeding.
...
- project/repo name exactly as it should exist on GitHub. Do not add or remove
  a `codi-` prefix automatically.
...
`./harness wire-infisical <repo-name>`.
...
After frontend scaffold or import finishes, mention that `codi-design-system`
can generate the optional project-owned tokens and `docs/design-system.md`.
...
Use (신규 프로젝트는 Question 5 답변에 따라 `--reset-git` 포함이 표준):

```bash
./harness init-project <repo-name> --org CODIWORKS-Engineer --mode <mode> --reset-git   # 신규
./harness init-project <repo-name> --org CODIWORKS-Engineer                             # 정규화/임포트
```

The script:

- runs `mise trust && mise install` only when the current Node.js runtime is not
  already ready.
- checks whether harness integration assets are already present and runs
  `./harness install` only when needed, unless `--skip-install` or
  `--force-install` changes that behavior.
- creates or reuses `<repo-name>` exactly as provided.
...
- Infisical Machine Identity (Universal Auth, TTL 0) and its Read grants on
  ...
  `/cloudflare/...`).
- Running `./harness wire-infisical <repo-name>` with
  `INFISICAL_PROJECT_ID`, `INFISICAL_CLIENT_ID`, and
