...
### E2E scaffolding (front + back)

The scaffold ships an e2e entry point per app so the e2e validation pipeline
(codi-dev-workflow `validation-pipeline.md`) has a suite to run — not only by
accident of the NestJS stack. App names follow the harness convention
(`apps/front`, `apps/back` — see monorepo-packages). Each app owns its e2e (no
root install):

- **front (Playwright):** add a `test:e2e` script running `playwright test`.
  Install Playwright with `npm` for React-only, `pnpm` for Next.js
  (monorepo-packages package-manager table).
- **back (Express, Supertest):** the `test:e2e` script above runs plain
  Supertest against the exported Express `app` (no `@nestjs/testing`).
- **back (NestJS):** the existing `test:e2e` Nest script already covers it.

E2E covers critical user-facing flows only — see the gate and the
"user-facing flow" definition in `.harness/policies/quality-gates.md`.
