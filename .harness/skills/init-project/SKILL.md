...
- One repository: the exact `<repo-name>` passed to `./harness init-project`.
...
1. Brainstorming: **Ask the user explicitly before any scaffold or import.**
   ...
   - If this working tree is a fresh harness clone starting a NEW project:
     confirm restarting git 히스토리 with `--reset-git` (default: yes). A new
     project must not keep the harness repo's full history and version
     branches; skipping this is only for the harness repo itself or an
...
If the target repository already exists and exactly one app is missing, prefer
...
```bash
./harness doctor
# 신규 프로젝트 (fresh harness clone) — git 히스토리 재시작 포함이 표준
./harness init-project <repo-name> --org CODIWORKS-Engineer --mode <split-front-back|next-fullstack> --reset-git
# 이미 초기화된 프로젝트의 정규화/임포트 — --reset-git 없이
./harness init-project <repo-name> --org CODIWORKS-Engineer
...
```bash
.harness/scripts/setup/init-project.sh <repo-name> --org CODIWORKS-Engineer
.harness/scripts/setup/wire-infisical.sh <repo-name> --org CODIWORKS-Engineer
```

Do not pass Infisical values to `init-project`. After the user finishes the
Infisical console work, run `./harness wire-infisical <repo-name>` with
...
Before the project's first feature, run `speckit-constitution` once to author
the project constitution (this activates the Constitution Check gates in
`speckit-plan` / `speckit-analyze`). `./harness doctor` flags a placeholder
constitution until this is done — including projects initialized before this
step existed.
