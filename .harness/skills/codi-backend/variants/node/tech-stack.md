...
### Express e2e (Supertest)

A user-facing flow e2e for Express is plain Supertest against the app — no
`@nestjs/testing`, no `AppModule`. Export the Express `app` (do not call
`listen` in the module) and drive it directly. When to write e2e:
`.harness/policies/quality-gates.md`.

```ts
// test/e2e/auth.e2e.test.ts
import request from "supertest";
import { app } from "../../src/app"; // app exported without listen()

test("login flow returns a session token", async () => {
  // intent: register → login → token issued
  await request(app)
    .post("/auth/register")
    .send({ email: "u@example.com", password: "s3cret-pass" })
    .expect(201);
...
Run with `mise run //apps/back:e2e` or, by default, `mise run e2e:changed`.
Critical flows only; an agent may author, a human reviews (see the principles
in `quality-gates.md`).
