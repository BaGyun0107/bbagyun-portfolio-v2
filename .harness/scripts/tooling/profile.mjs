...
const modes = {
  "planning-only": {
    description: "Planning artifacts only; both app surfaces are disabled.",
    apps: {
      front: { enabled: false, owns: [] },
      back: { enabled: false, owns: [] },
    },
    guidance: [
      "Do not create or modify apps/front/** or apps/back/**.",
      "This repository owns planning sources (feature definitions, sitemap, decisions).",
      "Implementation belongs in the downstream application repositories.",
    ],
  },
  "split-front-back": {
  ...
  "php-monolith": {
    description: "PHP monolith (e.g. Gnuboard mall); Node app surfaces are disabled, mall code lives under apps/<mall>/.",
    apps: {
      front: { enabled: false, owns: [] },
      back: { enabled: false, owns: [] },
    },
    guidance: [
      ...
      "Route PHP mall work to codi-gnuboard; do not use codi-frontend/codi-backend.",
    ],
  },
  "backend-only": {
...
#
# Supported modes:
# - planning-only: planning artifacts only; both app surfaces are disabled.
...
# - php-monolith: PHP monolith (e.g. Gnuboard); mall code under apps/<mall>/.
#
...
rules:
  planning-only:
    ...
    notes:
      ...
      - This repository owns planning sources only; implementation lives downstream.
  split-front-back:
    forbidden_paths: []
  ...
  php-monolith:
    forbidden_paths:
      - apps/front/**
      - apps/back/**
    owner_skill: codi-gnuboard
    notes:
      - Do not create files under apps/front or apps/back.
      - Mall code lives under apps/<mall>/ (any name other than front/back).
      - Route PHP mall work to codi-gnuboard, not codi-frontend/codi-backend.
`;
}
...
function checkProfile() {
  ...
  if (config) {
    ...
    const expectedForbidden =
      mode === "planning-only" || mode === "php-monolith"
...
if (command === "set") {
  ...
  printMode(modeArg);
  console.log("Record this architecture decision in the feature's specs/ directory when Spec Kit state exists.");
  process.exit(0);
}
