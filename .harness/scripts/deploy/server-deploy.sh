...
#     scp .harness/scripts/server-deploy.sh rocky@<server>:~/server-deploy.sh
...
# ── 인자 파싱 ──
if [ $# -lt 4 ]; then
    echo "사용법: $0 <BASE_PATH> <TAR_FILE> <APP_NAME> <ENV> [APP_TYPE] [--rollback]"
    echo ""
    echo "인자:"
...
# ── PM2 재시작 ──
# 주의: pm2 save 는 기동 성공 후 마지막에 한 번만 한다. 시작부에서 저장하면
# 고장 상태(stopped/errored 항목)까지 dump 에 박제되고, 데몬 재기동 시
# resurrect 가 그 항목을 불완전하게 되살려 이후 배포가
# "[PM2][ERROR] Process 0 not found" 로 반복 실패한다 (hipasshub-admin 실측).
restart_pm2() {
    ...
    if ! pm2 list | grep -q "$APP_NAME"; then
        ...
        # offline 항목은 데몬 내부 상태와 어긋난 잔여(stale)일 수 있어
        # 재시작 대신 제거 후 새로 시작한다 (self-heal).
        log "[$APP_NAME] offline 상태 → 잔여 항목 정리 후 start"
