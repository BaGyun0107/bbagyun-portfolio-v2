...
하지만 FULLTEXT 인덱스에는 한 가지 중요한 제약이 있었습니다. 당시 MyISAM 엔진의 설정상 인덱싱되는 단어의 최소 길이(\`ft_min_word_len\`)가 2로 제한되어 있어, 1글자 단어는 인덱스를 탈 수 없었습니다.
...
if (keywordLength >= 2) {
    // 2글자 이상: FULLTEXT BOOLEAN MODE 쿼리 실행 (초고속 역인덱스 검색)
    searchResults = await BookRepository.searchByFullText(searchKeyword);
} else {
    // 1글자: 기존 LIKE 쿼리 실행 (Fallback)
    searchResults = await BookRepository.searchByLike(searchKeyword);
}
\`\`\`

이 로직을 통해 2글자 이상의 핵심 키워드들은 FULLTEXT 인덱스를 타게 하여 검색 속도를 개선하고, 1글자 검색어는 기존 방식으로 처리되도록 방어 로직을 세웠습니다. 추가로, 정확히 일치하는 단락에 가중치를 부여하고 인접 단락을 비교하는 스코어링 로직을 결합해 검색의 정확도도 함께 끌어올렸습니다.

## The Result & Retrospective
### **성능 73% 향상과 최적화의 증명**
하이브리드 검색 라우팅을 적용한 결과, 기존 **1500ms에 달하던 쿼리 응답 시간을 400ms 수준으로 단축**시켰습니다. (실제 교재 데이터가 적재된 환경에서 자주 쓰이는 검색어들로 적용 전후의 평균 응답 시간을 비교한 값이며, 응답 시간은 검색어 길이와 매칭 단락 수에 따라 편차가 있습니다.) 제한된 환경과 리소스라는 제약 속에서도 애플리케이션 레이어와 DB 레이어의 특성을 결합하여 실질적인 비즈니스 임팩트를 만들어낸 의미 있는 최적화 경험이었습니다.
...
단순한 실수였지만, 이 경험 이후로는 외부 통신을 클라이언트에서 직접 처리하지 않고 **API Key·인증 같은 민감 정보는 항상 서버를 한 번 거치게 설계하는 것을 기본 원칙으로** 삼게 됐습니다.`
  },
  {
...
**결제/정산 불일치 방어**
이전 프로젝트에서 겪었던 결제 데이터 불일치 문제를 구조적으로 보완했습니다. JSON 파일 기반의 간이 Outbox 패턴과 트랜잭션 통제를 통해, 월평균 약 70건의 정산을 매월 실지급액과 대사한 기준에서 정산 누락이나 이중 지급 사고가 발생하지 않도록 관리했습니다.
...
이번 경험을 통해 **"프론트엔드 생태계 역시 서버와 네트워크 환경(HTTP 통신, 브라우저 보안 정책, 빌드 라이프사이클)에 대한 이해가 필요하다"**는 것을 배웠습니다. 이후로는 "왜 에러가 나지?"라며 단편적인 구글링에 의존하기보다, **"어떤 원리로 동작하는가?"를 먼저 확인하고 구조를 잡는** 쪽으로 작업 방식이 바뀌었습니다.`
  },
  {
...
초기 MVP 모델에서는 구현의 속도가 중요했기에 단일 Room과 In-memory 방식을 택했지만, 시스템이 성장함에 따라 아키텍처도 반드시 그에 맞춰 진화해야 합니다. 네트워크는 언제나 실패할 수 있다는 전제(Fallacies of distributed computing)하에 시스템을 방어적으로 설계해야 한다는 것을 체감했습니다.`
  },
  {
