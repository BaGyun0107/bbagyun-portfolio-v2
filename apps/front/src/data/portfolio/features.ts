...
#### 결과
- 예약 원본은 파트너사 시스템이 보유하고 당사는 그 API를 호출하는 창구를 담당했는데, 통신 로그상 동일 예약의 중복 호출 이력이 남지 않았고 파트너사로부터 중복 예약 관련 CS도 접수되지 않았습니다.
...
- **파일 I/O 약 95% 감소**: 요청당 1회 쓰기를 20건당 1회로 묶은 데서 나오는 산술적 감소치입니다 (1 - 1/20 ≈ 95%).
...
- **로그 라이프사이클:** 이후 Node.js 프로젝트에서 \`winston\` + \`winston-daily-rotate-file\`을 경험하며, 로그 레벨 분리와 자동 파기 정책의 필요성을 체득했습니다. ELK 스택 기반 중앙 집중형 수집 체계로 확장하여 운영 가시성을 확보할 것입니다.`
  },
  {
    ...
    title: '법문검색 엔진 구축',
    description:
      '한마음과학원 사이트의 77만 자의 비정형 데이터를 파싱하여 DB화하고, 레거시 DB 환경의 제약을 극복한 FTS + 동적 LIKE 하이브리드 검색 엔진을 고안해 응답 속도를 1500ms에서 400ms로 73% 개선한 프로젝트입니다.',
    iconName: 'Search',
    category: 'Fullstack',
    techStack: ['JavaScript', 'React', 'Node.js', 'Express', 'MySQL', 'Sequelize', 'GitHub Actions', 'PHP'],
    status: 'Production',    overview:
      '77만 자의 비정형 데이터를 파싱하여 DB화하고, 레거시 DB 환경의 제약을 극복한 FTS + 동적 LIKE 하이브리드 검색 엔진을 고안해 응답 속도를 1500ms에서 400ms로 73% 개선한 프로젝트입니다.',
...
- **보안 설계 성숙**: 이 프로젝트에서 JWT 인증을 처음 구현하며 \`httpOnly\`/\`secure\` 쿠키로 XSS는 방어했으나, CSRF 대응이 부족했습니다. 이후 프로젝트에서 SameSite 정책과 Double Submit Cookie 방식을 적용하며 보안 설계의 폭을 넓혔습니다.`
  },
  {
    ...
    description:
      '블랙스톤 벨포레리조트의 레거시(PHP) 환경에 SPA(React)를 결합한 점진적 공존 아키텍처를 설계하고, 결제 보상 트랜잭션과 백엔드 프록시를 도입해 결제-예약 불일치와 API Key 노출을 잡아간 프로젝트입니다. 운영 중 발생한 결제 장애를 원인별로 추적·보강한 경험을 담았습니다.',
    iconName: 'Laptop',
    category: 'Fullstack',
    techStack: ['JavaScript', 'React', 'Node.js', 'Express', 'MySQL', 'Sequelize', 'Jenkins', 'PHP'],
    status: 'Production',    overview:
      '레거시(PHP) 환경에 SPA(React)를 결합한 점진적 공존 아키텍처를 설계하고, 결제 보상 트랜잭션과 백엔드 프록시를 도입해 결제-예약 불일치와 API Key 노출을 잡아간 프로젝트입니다.',
...
- 앱 WebView에서 반복되던 세션 유실을 브릿지 인터페이스로 해소

### 챌린지 2: 결제 파편화로 인한 데이터 불일치 해결과 장애 대응

#### 문제 상황
기존 시스템은 "결제 승인 → DB 저장 → 외부 API 호출" 로직이 물리적으로 파편화되어 있었습니다. 결제는 PHP 서버에서, 예약(PMS)/티켓 생성 호출은 프론트에서 이루어지다 보니, 중간 단계가 실패하면 **결제는 됐지만 콘도·티켓이 생성되지 않는** 데이터 불일치가 간헐적으로 발생했습니다.
...
#### 해결 과정과 운영 중 드러난 장애 보강
오픈 후 현재까지 결제-예약 불일치는 **10건 미만**으로 관리되고 있으나, 처음부터 0건이었던 것은 아닙니다. 실제로 운영하며 세 가지 원인을 차례로 만나 보강했습니다.

1. **결제 응답이 프론트로 돌아오지 못한 케이스**: 결제는 PHP 서버에서 성공했지만 응답이 프론트로 넘어오지 않아 예약/티켓이 생성되지 않았습니다. 결제 승인 이후 PMS 예약/티켓 생성 실패·무응답·PHP 예외 상황에서도 서버가 자동으로 KSNET 결제 보상취소를 수행하도록 안정화했습니다.
2. **외부 API 장애로 보상취소가 동작하지 않은 케이스**: 외부 PMS 서버 장애가 약 한 시간(오전 11시~정오) 지속되며 무응답이 발생했고, 이때 결제는 됐지만 보상취소가 제대로 동작하지 않았습니다. \`tid\`를 받지 못한 경우에도 \`resvId\` 기반으로 결제 건을 추적해 취소할 수 있도록 fallback API와 프론트 재시도 로직을 추가했고, 보상취소 성공/실패·PMS 실패 원인·거래번호·예약 ID를 로그로 남겨 사후 추적이 가능하도록 개선했습니다.
3. **timeout을 너무 짧게 잡아 정상 요청을 실패로 오판한 케이스**: 외부 PMS의 평균 응답이 약 20초인데 예약 생성 timeout을 12초로 두어, 서버는 실패로 판단해 보상취소를 실행했지만 PMS는 뒤늦게 처리를 완료하면서 결제는 취소됐는데 예약은 생성되는 불일치가 확인됐습니다. PMS 생성 API의 timeout 정책을 60초로 조정해 정상 처리 중인 요청을 조기 실패로 판단하지 않도록 보완했습니다.

#### 결과
- 보상취소와 추적 로그를 단계적으로 보강해, 오픈 후 현재까지 결제-예약 불일치를 **10건 미만**으로 관리 (각 건은 원인 분석 후 구조적으로 보완)
- 결제 실패 시 사용자 입력 데이터를 복원해 재입력 불편을 제거
...
- **장애는 한 번에 끝나지 않는다**: 결제-예약 불일치는 보상취소 로직 하나로 끝나지 않았습니다. 응답 유실, 외부 API 장애 중 보상취소 실패, 짧은 timeout으로 인한 오판까지 운영하며 원인이 차례로 드러났고, 그때마다 fallback 추적(\`resvId\`)·timeout 정책(12초→60초)·실패 로그를 보강했습니다. "방어 로직을 짰다"보다 "장애가 날 때마다 추적 가능한 근거를 남기고 보강했다"가 더 중요한 운영 감각임을 체감했습니다.
...
- **API 계약 주도**: 파트너사에도 전례 없던 신규 티켓 기능을 개발하며, API 명세를 기다리는 구현자가 아닌 Response 구조를 먼저 제안하는 역할로 전환했습니다. API를 "시스템 간의 계약"으로 인식하고 기획 단계부터 구조를 리드하는 관점을 가지게 되었습니다.`
  },
  {
    ...
    description:
      '사내 서비스들의 파편화된 인증 체계를 통합하기 위해 NestJS 기반의 중앙 집중형 SSO 서버를 구축하고, JWT Payload와 Soft FK를 활용해 물리적으로 분리된 DB 환경에서 안전한 신뢰 모델을 설계한 프로젝트입니다.',
...
- 현 구조에서 Refresh Token 검증 시 발생하는 DB I/O는 트래픽이 증가할 경우 병목이 될 수 있습니다. Redis 캐싱 레이어 도입과 RTR(Refresh Token Rotation) 전략을 우선 적용해 보안과 성능을 함께 고도화하고 싶습니다.`
  },
  {
    ...
    description:
      '1인 백엔드로서 결제/정산 자동화 파이프라인(간이 Outbox 패턴)을 단독 설계하여 확인 가능한 범위에서 금융 데이터 불일치를 방어하고, 프론트엔드의 기능 단위 아키텍처 표준화까지 주도해 시스템 전체의 안정성과 유지보수성을 끌어올린 프로젝트입니다.',
    iconName: 'Store',
    category: 'Fullstack',
...
#### 결과
- 월평균 약 70건의 정산을 매월 실제 지급 금액과 대사(reconciliation)해 확인했고, 이 대사 기준으로 결제/정산 데이터 불일치는 발생하지 않았습니다. 서버 통제형 트랜잭션과 JSON Fallback으로 금융 데이터 불일치 리스크를 낮췄습니다.
...
- **결제/정산 데이터 불일치 방어:**
서버 통제형 트랜잭션과 JSON Fallback 시스템을 통해, 월평균 약 70건의 정산을 매월 실지급액과 대사한 기준에서 금융 데이터 불일치가 발생하지 않도록 관리했습니다.
- **조직 내 개발 표준 확립:**
...
#### 결과
- 모바일 결제 복귀 시 입력값을 localStorage(TTL)로 복원하고 가격·재고는 서버 재조회하는 흐름을 구현해, 결제창 이동 중 입력 데이터가 유실되지 않도록 설계
- 복귀 시점의 가격·재고 불일치를 감지해 이상 결제를 차단하는 로직 구현
- 보상 트랜잭션으로 PG 이중 청구 없이 롤백되도록 구성

> 이 프로젝트는 진행 중 중단되어 실제 운영 데이터로 검증되지는 못했습니다. 위 결과는 설계·구현 단계까지 완료한 범위이며, PL/PM을 겸하며 만든 기능과 설계 판단을 정리한 기록입니다.
...
- **도구보다 설계 원칙이 먼저다:** Redux 없이도 Context API와 라우터 격리만으로 도메인 경계를 충분히 강제할 수 있었습니다. 상태 관리 라이브러리를 선택하기 전에 스코프와 접근 제어 정책을 먼저 설계하는 습관을 갖게 되었습니다.`
  },
  {
    ...
    description:
      '1인 백엔드 및 PM/PL로서 요구사항 정의부터 Core 아키텍처 설계, Optimistic Lock 기반의 동시성 제어, 인프라 트러블슈팅까지 전 과정을 주도하며 모놀리식 환경에서 최적의 성능과 데이터 정합성을 이끌어낸 통합 플랫폼 프로젝트입니다.',
    iconName: 'Server',
    category: 'Backend',
    techStack: ['TypeScript', 'NestJS', 'MySQL', 'Prisma', 'Jenkins', 'PM2', 'Cloudflare'],
    status: 'In Progress',    overview:
      '1인 백엔드 및 PM/PL로서 요구사항 정의부터 Core 아키텍처 설계, Optimistic Lock 기반의 동시성 제어, 인프라 트러블슈팅까지 전 과정을 주도하며 모놀리식 환경에서 최적의 성능과 데이터 정합성을 이끌어낸 통합 플랫폼 프로젝트입니다.',
