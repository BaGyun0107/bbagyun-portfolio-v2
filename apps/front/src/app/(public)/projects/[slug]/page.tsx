...
import { getAllFeatures, getFeatureBySlug, getFeatureDetailBySlug, getAllInsights } from '@/data/portfolio';
import { getProjectStatusReason } from '@/data/portfolio/features';

export function generateStaticParams() {
  return getAllFeatures().map((f) => ({ slug: f.slug }));
}

// 사이드바는 sticky가 아니라 스크롤과 함께 사라지므로, 상단에서 훑어보는 용도로만 개수를 제한한다.
// 전체 목록은 본문 하단 '관련 인사이트' 섹션이 담당한다.
const SIDEBAR_INSIGHT_LIMIT = 5;
...
export default async function FeatureDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const statusReason = getProjectStatusReason(feature.slug);
  const relatedInsights = getAllInsights().filter((insight) => insight.featureSlug === slug);
      ...
      <div className='mb-8'>
        ...
        <div className='flex flex-col gap-2 mt-4 text-sm text-muted-foreground'>
          <div className='flex items-center gap-2'>
            ...
            <ProjectStatusBadge status={feature.status} />
        ...
        <div className='lg:col-span-2 space-y-8'>
          {detail ? (
            <ProjectDetailContent
              ...
              detail={detail}
              relatedInsights={relatedInsights.map(({ slug, title, excerpt }) => ({
                slug,
                title,
                excerpt
              }))}
            ...
            <div className='space-y-8'>
              ...
              {feature.content ? <MarkdownViewer content={feature.content} /> : null}
            </div>
          )}
        </div>
          ...
          <Card className='border-primary/20 bg-primary/5'>
            ...
            <CardContent className='space-y-4'>
              {relatedInsights.length > 0 ? (
                relatedInsights.slice(0, SIDEBAR_INSIGHT_LIMIT).map((insight) => (
                ...
                <p className='text-sm text-muted-foreground'>Check out the blog for more engineering deep dives.</p>
              )}
              <Link href='/insights' className='text-xs text-primary hover:underline inline-block mt-2'>
