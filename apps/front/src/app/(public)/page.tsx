...
import { getAllFeatures, getAllInsights } from '@/data/portfolio';
    ...
    <div className='space-y-16 py-10 md:py-20'>
      ...
      <section className='container mx-auto px-4 max-w-4xl text-center space-y-6'>
        ...
        <p className='text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed break-keep'>
          사용자를 위한 매끄러운 경험과 팀을 위한 견고한 아키텍처.
          <br className='hidden md:block' />
          시스템의 방향을 설계하고 기술의 깊이를 더하는 {Math.max(1, new Date().getFullYear() - 2023)}년 차 백엔드 중심
          풀스택 개발자입니다.
        </p>
          ...
          <div className='flex gap-12 px-6'>
            {[...techStack, ...techStack, ...techStack, ...techStack].map((tech, i) => (
              <div key={i} className='flex items-center gap-2 whitespace-nowrap'>
                ...
                <span className='text-lg font-medium text-foreground/80'>{tech.name}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <Link key={feature.id} href={`/projects/${feature.slug}`} className='block h-full group'>
              <Card className='h-full border-zinc-200 dark:border-zinc-800 transition-all duration-200 hover:border-zinc-400 dark:hover:border-zinc-600 hover:shadow-md flex flex-col p-6 gap-4'>
                ...
                <InlineMarkdown
                  ...
                  className='text-sm text-muted-foreground leading-relaxed line-clamp-2 group-hover:line-clamp-none transition-all'
                />
                ...
                <div className='mt-auto pt-2 flex flex-wrap gap-1.5'>
                  {feature.techStack.map((tech) => (
                    <span
                      key={tech}
                      className='text-xs text-muted-foreground bg-zinc-100 dark:bg-zinc-800 px-2 py-1 rounded'
                    >
                      {tech}
                    </span>
                  ))}
          ...
          {INSIGHTS.slice(0, 3).map((insight) => (
            <div key={insight.id} className='relative pl-8'>
              ...
              <InlineMarkdown
                ...
                className='text-muted-foreground mt-2 line-clamp-2 group-hover:line-clamp-none transition-all'
              />
            </div>
          ))}
        </div>
