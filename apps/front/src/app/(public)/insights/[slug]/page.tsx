...
export function generateStaticParams() {
  return getInsightRouteSlugs().map((slug) => ({ slug }));
}

export default async function InsightDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const canonicalSlug = resolveInsightAlias(slug);
  ...
  const typeLabel = insight.editorial ? INSIGHT_TYPE_LABELS[insight.editorial.type] : null;
        ...
        <div className={hasSidebar ? 'lg:col-span-2 space-y-8' : 'space-y-8'}>
          <article className='space-y-8'>
            <div className='space-y-4'>
              ...
              <h1 className='text-3xl md:text-5xl font-extrabold tracking-tight leading-tight [overflow-wrap:anywhere]'>
                {insight.title}
              </h1>
            ...
            <div className='prose prose-zinc dark:prose-invert max-w-none break-words'>
              <InlineMarkdown
                ...
                className='text-xl leading-relaxed text-muted-foreground block'
              />
              <div className='my-8 h-px bg-zinc-200 dark:bg-zinc-800' />

              {insight.visual ? <InsightVisual visual={insight.visual} /> : null}
          ...
          <div className='space-y-8'>
            <Card className='border-primary/20 bg-primary/5'>
              ...
              <CardContent className='space-y-4'>
                {relatedFeature && (
                  <Link
                    href={`/projects/${relatedFeature.slug}`}
                    className='block group p-3 rounded-lg border bg-background hover:bg-muted/50 hover:border-primary/50 transition-colors'
                  >
