...
import { getInsightTags, getInsightsByTag } from '@/data/portfolio';
...
export default async function InsightsTagsPage(props: { searchParams: Promise<{ tag?: string }> }) {
  ...
  const insights = currentTag ? getInsightsByTag(currentTag) : [];
  ...
  return (
    <div className='container mx-auto px-4 py-12 md:py-20 max-w-4xl'>
      <div className='mb-12'>
        ...
        <p className='text-xl font-medium text-zinc-600 dark:text-zinc-400'>관심 있는 주제의 태그를 선택해보세요.</p>
      </div>
      ...
      {!currentTag ? (
        ...
        <div className='space-y-8'>
          <div className='flex flex-col sm:flex-row sm:items-center gap-4 border-b pb-6'>
            <Link
              href='/insights/tags'
              className='px-4 py-2 border rounded-md text-sm font-medium hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors shrink-0'
            >
              &larr; 모든 태그 보기
            </Link>
                        ...
                        <div className='space-y-3'>
                          ...
                          <div className='flex gap-2 pt-2 flex-wrap'>
                            {insight.tags.map((tag) => (
                              <Badge
                                key={tag}
                                variant={tag === currentTag ? 'default' : 'secondary'}
