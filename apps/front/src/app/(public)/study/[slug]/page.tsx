...
export default async function StudyDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedInsights = getAllInsights().filter((insight) => insight.studySlug === slug);

  return (
    <div className='container mx-auto px-4 py-8 md:py-12 max-w-7xl'>
      <div className='mb-8'>
        <Link href='/study' className='text-sm text-muted-foreground hover:text-primary flex items-center gap-2 mb-4'>
          <ArrowLeft className='h-4 w-4' /> 뒤로가기
        </Link>
        <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
          <h1 className='text-3xl md:text-5xl font-bold tracking-tight'>{study.title}</h1>
        </div>
          ...
          <Card className='border-primary/20 bg-primary/5'>
            ...
            <CardContent className='space-y-4'>
              {relatedInsights.length > 0 ? (
                ...
                <p className='text-sm text-muted-foreground'>작성된 기술 블로그(인사이트) 글이 없습니다.</p>
              )}
