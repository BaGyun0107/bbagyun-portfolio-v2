...
export function MarkdownViewer({ content }: MarkdownViewerProps) {
  const components: Components = {
    ...
    ul: ({ node, ...props }) => <ul {...props} className='list-disc pl-6 space-y-1 marker:text-muted-foreground' />,
    ol: ({ node, ...props }) => <ol {...props} className='list-decimal pl-6 space-y-1 marker:text-muted-foreground' />,
    blockquote: ({ node, ...props }) => (
      <blockquote
        {...props}
        className='border-l-4 border-primary/30 bg-muted/40 px-4 py-2 my-4 rounded-r-md italic text-muted-foreground'
      />
    ),
    strong: ({ node, ...props }) => <strong {...props} className='font-bold text-zinc-900 dark:text-zinc-100' />,
    b: ({ node, ...props }) => <b {...props} className='font-bold text-zinc-900 dark:text-zinc-100' />,
    img: ({ node, ...props }) => (
      <figure className='my-6'>
        <img
          {...props}
          className='rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm w-full'
          loading='lazy'
        />
    ...
    code: ({ node, className, children, ...props }) => {
      ...
      return (
        <code
          {...props}
          className='px-1.5 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 font-mono text-sm text-rose-600 dark:text-rose-400'
        >
          {children}
        </code>
      );
